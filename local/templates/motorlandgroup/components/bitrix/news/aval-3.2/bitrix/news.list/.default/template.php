		...
		<div class="auto-run-wrap">
			...
			<div class="auto-run__box">
				<?foreach($arResult["ITEMS"] as $arItem):?>
					...
					<div class="auto-run-card__item shadow-box">
						<div class="auto-run-card__item--img">
							<?if ($arItem['PROPERTIES']['AN_RUK']['VALUE']) {?>
								<?foreach($arItem['PROPERTIES']['AN_RUK']['VALUE'] as $key=>$img) {?>
									<img src="<?=CFile::GetPath($img);?>" alt="">
									<?if ($key == 6) {break;}?>
								<?}?>
							<?} else {?>
								<?foreach($arItem['PROPERTIES']['AN_IMG']['VALUE'] as $key=>$img) {?>
									<img src="<?=$img?>" alt="">
									<?if ($key == 6) {break;}?>
								<?}?>
							<?}?>
						</div>
						<div class="auto-run-card__item--content">
							<div class="auto-run-card__item--name"><?=$arItem['NAME']?></div>
							...
							<div class="auto-run-card__item--price"><?=number_format($arItem['PROPERTIES']['AN_PRC']['VALUE'], 0, ',', ' ')?>&nbsp;₽</div>
							<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="btn-blue"><span>Подробнее</span></a>
						</div>
					</div>
				<?endforeach;?>
...
<?/*<pre><?=$arItem['PROPERTIES']['AN_KUZ']['VALUE']?></pre>
<pre></pre>
<pre><?=$arItem['PROPERTIES']['AN_KOM']['VALUE']?></pre>
<pre><?=$arItem['PROPERTIES']['AN_DVG']['VALUE']?></pre>
<pre><?=$arItem['PROPERTIES']['AN_PRC']['VALUE']?></pre>
<pre></pre>
<pre></pre>
<pre><?=$arItem['PROPERTIES']['AN_DTP']['VALUE']?></pre>
<pre><?=$arItem['PROPERTIES']['AN_PRI']['VALUE']?></pre>
<pre><?=$arItem['PROPERTIES']['AN_LIT']['VALUE']?></pre>
<pre></pre>
<pre><?=$arItem['PROPERTIES']['AN_KMS']['VALUE']?></pre>
<pre></pre>*/?>
